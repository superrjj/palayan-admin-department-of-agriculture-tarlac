// components/PestManagement/PestHeader
import React from 'react';
import { Search, Plus, BugOff } from 'lucide-react';

const PestHeader = ({ onAddNew, searchTerm, setSearchTerm }) => {
  return (
    <>
      {/* Header */}
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center py-6 space-y-4 lg:space-y-0">
            <div className="flex items-center space-x-3">
              <div className="p-2 bg-gradient-to-r from-green-600 to-emerald-500 rounded-lg">
                <BugOff className="w-6 h-6 lg:w-8 lg:h-8 text-white" />
              </div>
              <div>
                <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Pest</h1>
                <p className="text-gray-600 text-sm lg:text-base">Manage rice pests</p>
              </div>
            </div>
          </div>
        </div>

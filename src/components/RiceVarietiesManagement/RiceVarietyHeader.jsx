...
import { Search, Plus, Wheat} from 'lucide-react';

const RiceVarietyHeader = ({ onAddNew, searchTerm, setSearchTerm }) => {
  return (
    <>
      {/* Header */}
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center py-6 space-y-4 lg:space-y-0">
            <div className="flex items-center space-x-3">
              <div className="p-2 bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg">
                <Wheat className="w-6 h-6 lg:w-8 lg:h-8 text-white" />
              </div>
              <div>
                <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">Rice Varieties</h1>
                <p className="text-gray-600 text-sm lg:text-base">Manage rice variety entries</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 lg:p-6 mb-6 lg:mb-8 border border-gray-100">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0 gap-4">
          ...
          <button
            onClick={onAddNew}
            className="w-full md:w-auto bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white px-4 lg:px-6 py-3 rounded-lg flex items-center justify-center space-x-2 transition-all duration-200 transform hover:scale-105 shadow-lg text-sm lg:text-base font-medium"
          >
